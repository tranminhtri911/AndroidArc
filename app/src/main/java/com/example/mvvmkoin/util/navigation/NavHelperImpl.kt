...
import com.example.mvvmkoin.util.extension.generateTag
import com.example.mvvmkoin.util.extension.notNull
import com.example.mvvmkoin.util.extension.setAnimations
import com.example.mvvmkoin.util.extension.transact
...
constructor(
        ...
        private val fragListRoot: MutableList<ContainerFragment> = mutableListOf()) : NavHelper {
    ...
    override fun switchTab(tab: Int, fragment: Fragment,
        ...
        switchFragRoot.addOrReplaceFrag(fragment = fragment,
                isAddFrag = isAddFrag,
    ...
    override fun addFrag(fragment: Fragment, addToBackStack: Boolean,
            animateType: NavAnimateType, tag: String) {
        handler.post {
            curFragRoot.addOrReplaceFrag(fragment = fragment,
                    isAddFrag = true,
                    addToBackStack = addToBackStack,
                    animateType = animateType,
                    tag = tag)
    ...
    override fun replaceFrag(fragment: Fragment, addToBackStack: Boolean,
            animateType: NavAnimateType, tag: String) {
        handler.post {
            curFragRoot.addOrReplaceFrag(fragment = fragment,
                    isAddFrag = false,
    ...
    companion object {
        ...
        data class Builder(
            ...
            fun setup() = NavHelperImpl(tabs = tabs,
                    mainFraManager = mainFraManager,
                    mainContainerViewId = mainContainerViewId)

            fun tabs(tabs: List<Int>) = apply { this.tabs = tabs }

            fun mainFraManager(
                    fragManager: FragmentManager) = apply { this.mainFraManager = fragManager }

            fun mainContainerViewId(
                    resLayoutID: Int) = apply { this.mainContainerViewId = resLayoutID }
